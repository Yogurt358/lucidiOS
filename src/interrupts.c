...
idtr_t      idt_reg;
idt_entry_t i_entry[256] __attribute__((aligned(16)));
...
extern void isr_handler_C(stack_frame_t *frame) {
    ...
    switch(temp) {
        ...
        case 14:{
            ...
            kprintf("%x\n", faulting_address);
            kprintf("Faulting instruction: %x\n", frame->rip);
...
void init_interrupts() {
    ...
    set_gate(0,  0x8E, (uint64_t)isr0,  0, current_cs);  // #DE
    set_gate(1,  0x8F, (uint64_t)isr1,  0, current_cs);  // #DB
    set_gate(6,  0x8E, (uint64_t)isr6,  0, current_cs);  // #UD
    set_gate(8,  0x8E, (uint64_t)isr8,  0, current_cs);  // #DF
    set_gate(13, 0x8E, (uint64_t)isr13, 0, current_cs);  // #GP
    set_gate(14, 0x8E, (uint64_t)isr14, 0, current_cs);  // #PF
